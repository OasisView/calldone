...
2. The anonymous user is a real `auth.users` row with a real `auth.uid()`, so RLS
   ...
   **not** write `user_facts`, `script_edit_events`, or `script_feedback` (RLS-enforced AND
   skipped client-side).
3. Edge functions read the verified JWT claims and derive `{ userId, isAnonymous }`
   (`_shared/auth.ts`). Anonymous sessions are **force-coerced to demo mode** server-side (§6)
   and rate-limited per-IP and per-user (§8).
...
- Hosted dashboard: enable "Allow anonymous sign-ins" and set its rate limit to
  `RATE_LIMITS.anonymousSignInsPerIpPerHour` (this one limit is a dashboard setting, not the
  §8 RPC).
...
supabase/functions/
  _shared/
    ...
    auth.ts             ← getCaller(req): { userId, isAnonymous } from verified JWT claims
...
// Imported by Deno edge functions (../_shared/api-types.ts) AND the Vite app
// (via src/types/api.ts re-export). MUST stay dependency-free and
// side-effect-free: types, const arrays/maps/objects, and regex literals only.
// No Deno globals, no imports. Orchestrator-owned; do not edit.
// ============================================================================
...
export const API_ERROR_CODES = [
  ...
  "rate_limited",           // 429
  "internal_error",         // 500
  "not_implemented",        // 501 — frozen-but-cut code path (R12)
  "upstream_error",         // 502
  "quota_exceeded",         // 503
] as const;
...
// ------------------------------------------------------------- constants ---
/** E.164: + then 2-15 digits, no leading zero. Used by DB CHECK, zod, client. */
export const E164_REGEX = /^\+[1-9]\d{1,14}$/;

/** Webhook signed-request headers (R11). */
export const WEBHOOK_SIGNATURE_HEADER = "x-calldone-signature";   // hex HMAC-SHA256
...
/** Canonical numeric caps (R17). The ONLY normative copy of these numbers. */
export const LIMITS = {
  AUDIO_MAX_BYTES: 5242880,            // 5 MB
  AUDIO_MAX_SECONDS: 60,
  TTS_TEXT_MAX_CHARS: 800,
  CONVERSATION_MAX_TOTAL_CHARS: 30000,
  MESSAGE_MAX_CHARS: 4000,
  MESSAGES_MAX: 40,
  WEBHOOK_BODY_MAX_BYTES: 1048576,     // 1 MB
  TRANSCRIPT_STORED_MAX_CHARS: 100000,
  POLL_INTERVAL_MS: 5000,
  POLL_TIMEOUT_MS: 120000,
  DEMO_DEFAULT_DELAY_SECONDS: 30,
} as const;

/** Canonical rate-limit numbers (R6). Enforced via public.rate_limit_hit (§8 of api.md). */
export const RATE_LIMITS = {
  makeCall: {
    perIpPerHour: 5,
    anonPerUidPerDay: 10,
    authedPerUidPerDay: 20,
  },
  whisperTranscribe: {
    perUidPerHour: 30,
    perIpPerHour: 60,
  ...
  elevenlabsTts: {
    requestsPerUidPerHour: 20,
    requestsPerIpPerHour: 60,
    charsPerUidPerDay: 1500,
    charsPerMonthGlobal: 9000,
  },
  /** Supabase Auth dashboard setting, not the RPC. */
  anonymousSignInsPerIpPerHour: 10,
} as const;
...
// --------------------------------------------------- whisper-transcribe ---
/** POST multipart/form-data. Field names for the form. */
export const TRANSCRIBE_AUDIO_FIELD = "audio";       // File/Blob: webm, ogg, mp4/m4a, or wav;
                                                     // <= LIMITS.AUDIO_MAX_BYTES; <= LIMITS.AUDIO_MAX_SECONDS
...
export interface GeminiConversationRequest {
  /** Full history, oldest first. 1..LIMITS.MESSAGES_MAX items; total text
   *  <= LIMITS.CONVERSATION_MAX_TOTAL_CHARS. Last item MUST be role "user".
   *  The system prompt is built SERVER-SIDE only (disclosures, user facts);
   *  clients can never inject or override it. */
...
export interface ExtractedFact {
  key: string;        // snake_case, matches /^[a-z][a-z0-9_]{1,63}$/
...
export interface FinalizedScript {
   ...
   *  ("AI assistant calling on behalf of <name>", "this call may be recorded"). */
  script_text: string;              // 40..4000 chars
  call_purpose: string;             // 3..200 chars, e.g. "Refill blood pressure prescription"
  target_phone_hint: string | null; // E.164 or null if the user never said a number
  extracted_facts: ExtractedFact[]; // 0..10 items
}
...
// ------------------------------------------------------- elevenlabs-tts ---
export const TTS_VOICES = ["default"] as const; // extensible later; server maps to real ElevenLabs voice ids
...
// 502/503 => client returns null from synthesizeSpeech() and falls back to
// browser SpeechSynthesis with a notice (R7). Never a 200-with-fallback-payload.

// ------------------------------------------------------------ make-call ---
export interface MakeCallRequest {
  script_id: string;    // uuid of a call_scripts row owned by the caller (RLS-enforced)
  phone_number: string; // E.164
  // NOTE: deliberately NO client-supplied demo/real flag. Server decides (api.md §6).
}

export interface MakeCallResponse {
  call_log_id: string;  // uuid of the call_logs row created by this function
  call_id: string;      // "demo_<uuid>" in demo mode
  mode: CallMode;       // what the server actually decided ("demo" always, in this slice)
  status: "initiated";
  /** Seconds until the simulated webhook fires (demo); null for real calls (P6). */
  estimated_completion_seconds: number | null;
}

// -------------------------------------------------------- schedule-call ---
// P6 — NOT BUILT OR DEPLOYED IN THIS SLICE (R10). Types frozen here only so the
// contract is complete; clients have SELECT-only access to scheduled_calls.
...
export interface ScheduleCallResponse {
  scheduled_call_id: string; // uuid of the scheduled_calls row
  status: "pending";
  scheduled_for: string;     // normalized UTC ISO 8601
}

// --------------------------------------------------------- call-webhook ---
/** Correlation metadata. Set by make-call and included verbatim in simulated
 *  demo payloads (Bland will echo it back in P6). NEVER a source of identity —
 *  cross-check only (R4). */
 ...
 *  use EXACTLY this shape and flow through the same parser. Unknown extra
 *  fields are ignored (zod .passthrough()). */
...
export interface AppointmentDetails {
  ...
  start_iso: string;        // ISO 8601 WITH offset, e.g. "2026-06-15T14:15:00-04:00"
  end_iso: string | null;   // null => assume 30 min when building .ics
  location: string | null;
  notes: string | null;
  confidence: number;       // 0..1; store but flag < 0.6 as "unconfirmed" in UI
}
```
...
| Auth | `JWT (anon ok)` |
| Request | `multipart/form-data`: field `audio` (required; `audio/webm`, `audio/ogg`, `audio/mp4`, `audio/wav`; ≤ `LIMITS.AUDIO_MAX_BYTES`; ≤ `LIMITS.AUDIO_MAX_SECONDS`), field `language` (optional, ISO-639-1) |
...
| Rate limit | `RATE_LIMITS.whisperTranscribe` (per-uid/hour + per-IP/hour) |
...
| Rate limit | `RATE_LIMITS.geminiConversation` (per-uid/hour + per-IP/hour) |
...
| Rate limit | `RATE_LIMITS.elevenlabsTts`: per-uid/hour + per-IP/hour request counts, plus char budgets per-uid/day and global/month (§8) |
...
| Rate limit | `RATE_LIMITS.makeCall` (per-IP/hour + anon-uid/day + authed-uid/day) |
...
const payload: CallWebhookPayload = {
  call_id: args.demoCallId,                 // "demo_<uuid>" — same value stored in call_logs.bland_call_id
  to: args.phoneNumber,                     // the number the user entered (never dialed)
  from: "+15555550100",                     // fixed fictional caller id (555 range, undialable)
  completed: true,
  status: "completed",
...
```
public.rate_limit_hit(bucket text, key text, window text, amount int default 1) returns int
```
...
| Bucket (`bucket` arg) | Key | Window | Constant (`RATE_LIMITS.…`) | Value |
|---|---|---|---|---|
| `make_call:ip` | client IP | hour | `makeCall.perIpPerHour` | 5 |
| `make_call:uid_anon` | anonymous uid | day | `makeCall.anonPerUidPerDay` | 10 |
| `make_call:uid` | authed uid | day | `makeCall.authedPerUidPerDay` | 20 |
| `whisper:uid` | uid | hour | `whisperTranscribe.perUidPerHour` | 30 |
| `whisper:ip` | client IP | hour | `whisperTranscribe.perIpPerHour` | 60 |
| `gemini:uid` | uid | hour | `geminiConversation.perUidPerHour` | 60 |
| `gemini:ip` | client IP | hour | `geminiConversation.perIpPerHour` | 120 |
| `tts:uid` | uid | hour | `elevenlabsTts.requestsPerUidPerHour` | 20 |
| `tts:ip` | client IP | hour | `elevenlabsTts.requestsPerIpPerHour` | 60 |
| `tts_chars:uid` | uid | day | `elevenlabsTts.charsPerUidPerDay` | 1500 |
| `tts_chars:global` | `"global"` | month | `elevenlabsTts.charsPerMonthGlobal` | 9000 |
| Anonymous sign-ins | client IP | hour | `anonymousSignInsPerIpPerHour` (Supabase Auth dashboard setting, not the RPC) | 10 |
